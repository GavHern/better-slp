...
function openNoteTaker() {
  chrome.storage.sync.get('darkMode', function (items) {
    ...
    var focusAreaId = new URL(window.location.href).pathname.split('my/focusareas/')[1]; // Find ID of focus area based on the URL pathname

    var noteTakerEmbedURL = chrome.runtime.getURL("html/notetaker.html?id=".concat(focusAreaId, "&dark=").concat(items.darkMode)); // Get the chrome-extension url and add the id & theme as a parameter
